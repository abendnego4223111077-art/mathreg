...
<div class="page">
    ...
    <div class="content-card gallery-card">
        ...
        <div class="content-card" style="margin-top:1.5rem; text-align:right;">
            <a href="{{ route('presentasi2') }}" class="btn-next">
                Lanjut ke Presentasi 2 →
            </a>
        </div>
    </div>
</div>
...
<script>
    function goSubStep(step) {
        ...
        if (step === 3) {
            document.getElementById('clue-grid').scrollIntoView({ behavior: 'smooth', block: 'start' });
        }
    }
    ...
    function plotGarisOrientasi() {
        ...
        const b = 0.2;
        const x1 = pad;
        const y1 = canvas.height - pad - ((m * minX + b - minY) / (maxY - minY)) * h;
        const x2 = pad + w;
        const y2 = canvas.height - pad - ((m * maxX + b - minY) / (maxY - minY)) * h;
    ...
    function openClue(index) {
        if (openedClues >= maxClues && openedClues <= index) {
            return;
        }
